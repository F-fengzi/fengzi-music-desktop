...
$(function(){
    ...
    $(".music-list").on("click",".list-loadmore", function() {
        ...
        $(".list-loadmore").html('加载中=≡Σ(((つ•̀ω•́)つ');
        ajaxSearch();
    });
    ...
    $("#sheet").on("click",".sheet-cover,.sheet-name", function() {
        ...
        if(musicList[num].item.length === 0 && musicList[num].creatorID) {
            layer.msg('列表读取中 (=ﾟωﾟ)ﾉ', {icon: 16,shade: 0.01,time: 500}); // 0代表加载的风格，支持0-2
            // ajax加载数据
    ...
    $("#sheet").on("click",".login-in", function() {
        ...
        function(val, index){   // 输入后的回调函数
            if(isNaN(val)) {
                layer.msg('ID号是一串数字哦 (*/ω＼*)~',{anim: 6,icon: 0});
                return false;
            }
    ...
    $("#sheet").on("click",".login-refresh", function() {
        ...
        layer.msg('正在刷新歌单 (oﾟvﾟ)ノ',{icon: 16,icon: 1,time: 500});
        clearUserlist();
    });
    ...
    if((mkPlayer.coverbg === true && !rem.isMobile) || (mkPlayer.mcoverbg === true && rem.isMobile)) { // 开启了封面背景
        ...
        if(rem.isMobile) {  // 移动端采用另一种模糊方案
            $('#blur-img').html('<div class="blurred-img" id="mobile-blur"></div><div class="blur-mask mobile-mask focus-indicator"></div>');
        } else {
            // 背景图片初始化
            $('#blur-img').backgroundBlur({
                ...
                overlayClass : 'blur-mask focus-indicator', // 覆盖背景区class，可用于遮罩或额外的效果
                // duration: 0, // 图片淡出时间
...
function searchBox() {
    ...
    '        <input type="text" name="wd" id="search-wd" placeholder="试试搜索歌名/歌手/专辑名~" autofocus>' + 
    '        <button class="search-submit" type="submit">搜网易</button>' + 
    '    </div>' + 
    ...
    '       <label id="last-label"><input type="radio" name="source" value="baidu"> 百度</label>' + 
    '   </div>' + 
    '</div></form>';
    layer.open({
        type: 1,
...
function changeSearch(value){
    if (value == 'netease') {
        document.getElementsByClassName("search-submit")[0].innerHTML = "搜网易";
    }else if (value == 'tencent') {
        document.getElementsByClassName("search-submit")[0].innerHTML = "搜QQ";
    }else if (value == 'kugou') {
        document.getElementsByClassName("search-submit")[0].innerHTML = "搜酷狗";
    }else if (value == 'xiami') {
        document.getElementsByClassName("search-submit")[0].innerHTML = "搜虾米";
    }else if (value == 'baidu') {
        document.getElementsByClassName("search-submit")[0].innerHTML = "搜百度";
    }
}
...
function searchSubmit() {
    ...
    if(!wd) {
        layer.msg('空的让我怎么搜啊 (⊙ˍ&nbsp;&nbsp;⊙)？', {anim:6,icon:0, offset: 't'});
        $("#search-wd").focus();
        return false;
    }
...
function loadList(list) {
    if(musicList[list].isloading === true) {
        layer.msg('列表读取中ヾ(•ω•`。)', {icon: 16,shade: 0.01,time: 500});
        return true;
    }
...
function addListbar(types) {
    ...
    switch(types) {
        case "more":    // 还可以加载更多
            html = '<div class="list-item text-center list-loadmore list-clickable" title="愣着干嘛" id="list-foot">再加载些！</div>';
        break;
        
        case "nomore":  // 数据加载完了
            html = '<div class="list-item text-center" id="list-foot">真的全都加载完了(ಥ _ ಥ)</div>';
        break;
        
        case "loading": // 加载中
            html = '<div class="list-item text-center" id="list-foot">正在拼命加载列表ヾ(≧ ▽ ≦)ゝ</div>';
        break;
        
        case "nodata":  // 列表中没有内容
            html = '<div class="list-item text-center" id="list-foot">这里还什么都没有呢ヾ(•ω•`)o</div>';
        break;
        
        case "clear":   // 清空列表
            html = '<div class="list-item text-center list-clickable" id="list-foot" onclick="clearDislist();">清空！全清了( ￣▽￣)/</div>';
        break;
    }
    rem.mainList.append(html);
}
...
function sheetBar() {
    ...
    if(playerReaddata('uid')) {
        ...
        barHtml = '<span class="login-btn login-in">点击同步我的网易云歌单</span>';
    }
