...
function ajaxSearch() {
    if(rem.wd === ""){
        layer.msg('啥都不填你说我搜什么嘛 ( ･´ω`･ )', {anim:6, icon:0});
        return false;
    }
    
    if(rem.loadPage == 1) { // 弹出搜索提示
        var tmpLoading = layer.msg('努力搜索中_(:з」∠)_', {icon:16, shade:0.01});
    }
    ...
    $.ajax({
        ...
        success: function(jsonData){
            ...
            if(rem.loadPage == 1)   // 加载第一页，清空列表
            {
                ...
                {
                    layer.msg('竟然没找到这首歌 ┐(ﾟ～ﾟ)┌', {icon:2, anim:6});
                    return false;
                }
                musicList[0].item = [];
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('获取搜索结果失败了 (ノДＴ) - ' + XMLHttpRequest.status, {icon:2});
...
function ajaxUrl(music, callback)
{
    ...
    layer.msg('正在拼命加载ヾ(ｏ･ω･)ﾉ...', {icon:16, shade:0.01});
    $.ajax({ 
        type: mkPlayer.method, 
        url: mkPlayer.api,
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('去偷歌的路上被打回来了 ∑(ﾟДﾟノ)ノ - ' + XMLHttpRequest.status, {icon:2});
            console.error(XMLHttpRequest + textStatus + errorThrown);
            setTimeout(function(){contact();},1000);
        }   // error 
    }); //ajax
...
function ajaxPlayList(lid, id, callback) {
    ...
    $.ajax({
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('歌单没读取成功 (´⊙ω⊙`) - ' + XMLHttpRequest.status, {icon:2});
            setTimeout(function(){contact();},1000);
            console.error(XMLHttpRequest + textStatus + errorThrown);
...
function ajaxLyric(music, callback) {
    ...
    $.ajax({
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('歌词没拿到啊 o(╥﹏╥)o - ' + XMLHttpRequest.status, {icon:2});
            console.error(XMLHttpRequest + textStatus + errorThrown);
            callback('', music.lyric_id);    // 回调函数
            setTimeout(function(){contact();},1000);
        }   // error   
...
{
    var tmpLoading = layer.msg('试图登录中 (゜-゜)', {icon:16, shade:0.01});
    $.ajax({
        type: mkPlayer.method,
        url: mkPlayer.api,
        ...
        success: function(jsonData){
            ...
            if(jsonData.playlist.length === 0 || typeof(jsonData.playlist.length) === "undefined")
            {
                ...
                layer.msg('(・ω<) 登录成功了！ '+rem.uname, {icon:1});
                // 记录登录用户
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            layer.msg('歌单同步失败！迷路了 (ﾒ｀ﾛ´)/ - ' + XMLHttpRequest.status, {icon:2});
            console.error(XMLHttpRequest + textStatus + errorThrown);
            setTimeout(function(){contact();},1000);
        }   // error
    });//ajax
